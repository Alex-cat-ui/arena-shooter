extends Node

const TestHelpers = preload("res://tests/test_helpers.gd")
const ENEMY_SCENE := preload("res://scenes/entities/enemy.tscn")
const ENEMY_UTILITY_BRAIN_SCRIPT := preload("res://src/systems/enemy_utility_brain.gd")

var embedded_mode: bool = false
var _t := TestHelpers.new()


func _ready() -> void:
	if embedded_mode:
		return
	var result := await run_suite()
	get_tree().quit(0 if bool(result.get("ok", false)) else 1)


func run_suite() -> Dictionary:
	print("")
	print("============================================================")
	print("COMBAT NO-LOS NEVER HOLD_RANGE TEST")
	print("============================================================")

	await _test_combat_no_los_never_hold_range()

	_t.summary("COMBAT NO-LOS NEVER HOLD_RANGE RESULTS")
	return {
		"ok": _t.quit_code() == 0,
		"run": _t.tests_run,
		"passed": _t.tests_passed,
	}


func _test_combat_no_los_never_hold_range() -> void:
	if RuntimeState:
		RuntimeState.player_hp = 100
	var world := Node2D.new()
	add_child(world)

	var player := CharacterBody2D.new()
	player.add_to_group("player")
	player.global_position = Vector2(260.0, 0.0)
	world.add_child(player)

	var enemy := ENEMY_SCENE.instantiate() as Enemy
	enemy.global_position = Vector2(0.0, 0.0)
	world.add_child(enemy)
	await get_tree().process_frame
	await get_tree().physics_frame
	enemy.initialize(4801, "zombie")
	enemy.set_runtime_budget_scheduler_enabled(true)
	enemy.debug_force_awareness_state("COMBAT")
	enemy.runtime_budget_tick(0.1)

	var blocker := _spawn_blocker(world, Vector2(120.0, 0.0), Vector2(32.0, 640.0))
	_t.run_test("setup: blocker exists", blocker != null)
	player.global_position = Vector2(300.0, 140.0)
	await get_tree().physics_frame

	enemy.runtime_budget_tick(0.75)
	var snapshot_during_grace := enemy.get_debug_detection_snapshot() as Dictionary
	var intent_during_grace := enemy.get_current_intent() as Dictionary
	var type_during_grace := int(intent_during_grace.get("type", -1))
	_t.run_test("during grace: no LOS", not bool(snapshot_during_grace.get("has_los", true)))
	_t.run_test(
		"during grace: no HOLD_RANGE in COMBAT no-LOS",
		type_during_grace != ENEMY_UTILITY_BRAIN_SCRIPT.IntentType.HOLD_RANGE
		and type_during_grace == ENEMY_UTILITY_BRAIN_SCRIPT.IntentType.PUSH
	)

	enemy.runtime_budget_tick(0.90)
	var snapshot_after_grace := enemy.get_debug_detection_snapshot() as Dictionary
	var intent_after_grace := enemy.get_current_intent() as Dictionary
	var type_after_grace := int(intent_after_grace.get("type", -1))
	_t.run_test("after grace: still no LOS", not bool(snapshot_after_grace.get("has_los", true)))
	_t.run_test(
		"after grace: still no HOLD_RANGE in COMBAT no-LOS",
		type_after_grace != ENEMY_UTILITY_BRAIN_SCRIPT.IntentType.HOLD_RANGE
	)

	world.queue_free()
	await get_tree().process_frame


func _spawn_blocker(parent: Node2D, pos: Vector2, size: Vector2) -> StaticBody2D:
	var body := StaticBody2D.new()
	body.position = pos
	var shape := CollisionShape2D.new()
	var rect := RectangleShape2D.new()
	rect.size = size
	shape.shape = rect
	body.add_child(shape)
	parent.add_child(body)
	return body
