...
func _test_role_lock_and_triggered_reassign() -> void:
	...
	enemy.call("_update_combat_role_runtime", 0.1, true, false, 4.0, false, 0, assignment)
	...
	for _i in range(11):
		enemy.call("_update_combat_role_runtime", 0.1, false, false, 4.0, false, 0, assignment)
	var snap_lost_los := enemy.get_debug_detection_snapshot() as Dictionary

	# Early trigger: stuck > 1.2s
	for _i in range(13):
		enemy.call("_update_combat_role_runtime", 0.1, true, true, 0.5, false, 0, assignment)
	var snap_stuck := enemy.get_debug_detection_snapshot() as Dictionary

	# Early trigger: path_failed >= 3
	for _i in range(3):
		enemy.call("_update_combat_role_runtime", 0.1, true, true, 0.5, true, 0, assignment)
	var snap_path_failed := enemy.get_debug_detection_snapshot() as Dictionary

	# Early trigger: target_room_changed
	enemy.call("_update_combat_role_runtime", 0.1, true, false, 4.0, false, 1, assignment)
	enemy.call("_update_combat_role_runtime", 0.1, true, false, 4.0, false, 2, assignment)
